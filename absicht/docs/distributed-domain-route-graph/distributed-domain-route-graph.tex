...
into a fixed architecture. Two independent manuscripts by the same author
supply, without citing one another or the receiver formalism directly,
exactly the missing pieces: \emph{Federated Context Closure}
...
provable synchronisation threshold. This paper proves that these three
results compose: that a route graph is itself a bounded receiver in the
sense of \citet{sachikonye-accountable-compilation}, that water-filling is
the continuous relaxation of the Cascade Theorem's discrete knapsack as the
number of consulted receivers grows, that a federation of phase-locked,
individually-opaque receivers inherits a floor that sharpens geometrically
in the number of \emph{locked} members while correctly excluding, rather
than averaging in, members that fail to lock, and that the whole assembly is
again a bounded receiver — closing the loop, so that a federation of
federations is not a new kind of object. Six computational experiments
validate each composed claim against exact or simulated ground truth.
\end{abstract}
...
members and by making the exclusion of unlocked members precise. Sections
\ref{sec:elimination} and~\ref{sec:seam} close the loop: a receiver in this
...
Six experiments, following the exact discipline of
...
\texttt{validation/results/*.json}). The six experiments and their panels
...
(\Cref{thm:seam}, \Cref{fig:panel-seam}), and crowd-sharpening under
route-graph selection (\Cref{fig:panel-crowd-routing}).
...
identical to the one \Cref{thm:route-audit} closes for a pair. The
phase-lock threshold $K_c$ is treated here as a fixed parameter of the
Kuramoto model; whether it should instead be learned per-domain, and what
happens to \Cref{thm:phaselock-floor}'s bound if $K_c$ is itself estimated
rather than known, is not addressed. Finally, \Cref{thm:waterfill-cascade}'s
convergence to the discrete knapsack optimum is asymptotic in the number of
receivers and the relative smallness of per-item cost; a non-asymptotic,
finite-$k$ characterisation of the gap — rather than the limiting argument
given here — would sharpen \Cref{cor:when-waterfill}'s practical guidance on
