...
to resolve a query against an incompletely known domain. Training data
selection is, properly understood, floor minimisation by choice of task
distribution. Model composition is floor minimisation by choice of receiver
set and routing policy. Output checking is floor minimisation by choice of
...
sample-complexity separation and specify a minimal declarative interface (a
\emph{Domain Contract}) sufficient to acquire a low-rank adapter for any
domain meeting the theorem's hypotheses.
...
minimise the federation's floor is exactly a $0$--$1$ knapsack problem, hence
solvable exactly in pseudo-polynomial time and near-exactly by a greedy rule.
We prove the \textbf{Minimum Loop Theorem}: no directed acyclic graph of
pairwise checks among receivers can certify an output at a floor below the
floor of its least-checked member, and a minimal sufficient certifying
structure is a directed cycle of at least three receivers.
...
knowledge representations admit no shared decoding map, and ask how such a
pair certifies agreement without either disclosing its internal state. We
construct a four-part relaxation --- the answer each receiver gives, and the
further query each receiver's own answer would provoke --- and prove the
\textbf{Quiescence Theorem}: the relaxation reaches a fixed point if and only
...
\textbf{Verification-Floor Theorem}: the four-part relaxation of
\Cref{part:verify}, run to quiescence or to a declared non-convergence,
\emph{is itself a bounded receiver} in the sense of \Cref{part:compose}, with
...
produces low-rank domain adapters, a serving pipeline
(\Cref{part:compose}) that routes and federates them under budget with a
provably sufficient validation topology, and a cross-domain audit layer
(\Cref{part:verify}--\Cref{sec:extension}) that certifies agreement between
independently-trained adapters --- or between an adapter and an external
knowledge source --- without requiring either to expose its internal
...
the standard non-expansiveness of metric projection onto a convex set; we
state it as an axiom because the framework in this paper is meant to apply
...
support). Let $\Delta := \inf_{x\in\Cands^\star\setminus\Cands_c}
...
\Cref{ax:monotone}. Let a model class have VC dimension $d$. Then achieving
...
A low-rank adapter on a shared, frozen base model is then trained on the
...
architecture; it is a counting fact about maps between finite sets.
\end{remark}
...
if the others are not. Taking the \emph{intersection} instead --- requiring
...
stated knapsack problem, with the stated complexity and approximation
guarantee by standard results on $0$--$1$ knapsack.
\end{proof}
...
graph $G=(\Fed,E)$ where $(\Rec_i,\Rec_j)\in E$ means $\Rec_j$ evaluates
...
where a \emph{terminal} receiver is one with no outgoing check edge.
\end{theorem}
...
fixed-point theorem applied to the composite contraction $\mathcal{V}$ on the
...
implement, cannot detect this; the four-column construction can, because it
compares what each answer \emph{provokes} in its own receiver, not merely
the answer's surface value.
\end{remark}
...
under the receivers' own follow-up policies.
\end{definition}
...
source (a graph-like index over the group's own records) that exposes a
...
provably well-behaved quantity.
