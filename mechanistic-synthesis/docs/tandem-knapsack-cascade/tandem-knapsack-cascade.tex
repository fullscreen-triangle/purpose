...
\title{\textbf{Carry the Uncertainty, Not the Knowledge:\\[3pt]
A Tandem Knapsack--Cascade Calculus for the Context of a\\
Finite Reasoning Agent}}
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{panel1_floor.png}
\caption{\textbf{Floor} (\cref{thm:floor}). \textbf{(A)}~Residue $\res(u)$ of
every step against graph size; the dashed line is the floor $\flo$, which no
residue crosses. \textbf{(B)}~Distribution of $\res(u)/\flo$, supported entirely
at and above $1$. \textbf{(C)}~Residue against step degree, coloured by
$\res(u)/\flo$; the floor line bounds the cloud below. \textbf{(D)}~Residue over
$(\text{steps},\flo)$ with the floor plane $\res=\flo$; every point lies on or
above it.}
\label{fig:panel1}
\end{figure*}
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{panel2_invariance.png}
\caption{\textbf{Residue invariance} (\cref{thm:invariant}).
\textbf{(A)}~Residue after a random re-encoding against residue before: exactly
on the diagonal. \textbf{(B)}~Label index after against before: scattered off the
diagonal---labels permute. \textbf{(C)}~Distribution of the residue discrepancy,
concentrated at zero. \textbf{(D)}~The $(\text{before},\text{after},\text{discrepancy})$
cloud sitting on the zero-discrepancy plane.}
\label{fig:panel2}
\end{figure*}

\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{panel3_necessity.png}
\caption{\textbf{Necessity vs.\ reachability}
(\cref{thm:necessity,rem:necessity-scope}). \textbf{(A)}~$\abs{\nec(\goal)}$
against $\abs{\reach(\goal)}$ on random graphs: every point on or below the
diagonal ($\nec\subseteq\reach$). \textbf{(B)}~Per-position contribution along a
chain: interior steps are necessary, the terminal leaf (last bar) contributes
zero. \textbf{(C)}~Distribution of the gap $\abs{\reach}-\abs{\nec}$.
\textbf{(D)}~Necessity flag over $(\text{position},\text{chain length})$: the
terminal leaf (lower band) is never necessary; interiors (upper band) always
are.}
\label{fig:panel3}
\end{figure*}
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{panel4_freedrop.png}
\caption{\textbf{Free drop} (\cref{thm:freedrop}). \textbf{(A)}~Reachable-set
size after dropping a purposeless step against before: exactly on the diagonal.
\textbf{(B)}~Change in the reachable set on each drop, concentrated at zero.
\textbf{(C)}~Distribution of purposeless steps per graph. \textbf{(D)}~The change
$\Delta$ over $(\abs{\reach},\text{number of drops})$, lying on the zero plane.}
\label{fig:panel4}
\end{figure*}

\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{panel5_knapsack.png}
\caption{\textbf{Knapsack} (\cref{thm:knapsack,thm:greedy}). \textbf{(A)}~Greedy
value against the exact optimum: on or below the diagonal, with the exact DP
optimal. \textbf{(B)}~Greedy/exact ratio against $\cost_{\max}/\budget$; the
dashed line is the guaranteed bound $1-\cost_{\max}/\budget$, which every point
clears. \textbf{(C)}~Distribution of the greedy/exact ratio, concentrated near
$1$. \textbf{(D)}~The ratio over $(\cost_{\max}/\budget,\budget)$ above the
guarantee surface.}
\label{fig:panel5}
\end{figure*}
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{panel6_cascade.png}
\caption{\textbf{Cascade} (\cref{thm:cascade}). \textbf{(A)}~Frames visited
against $N$ (log axis) for $k\in\{2,3,4\}$: the $\lceil\log_k N\rceil+1$ law.
\textbf{(B)}~Working set against total history length $M$ (log axis): flat---no
dependence on $M$. \textbf{(C)}~Per-goal cost, flat $\mathcal{O}(N)$ against the
cascade's $\log_k N$. \textbf{(D)}~Frames over $(k,\log_{10}N)$ with the
$\lceil\log_k N\rceil+1$ surface.}
\label{fig:panel6}
\end{figure*}

\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{panel7_separation.png}
\caption{\textbf{Separation} (\cref{thm:separation}). \textbf{(A)}~The diamond:
the goal reaches $u_1,u_2$ (upper/lower), both feeding the resolver $r$; the
redundant leg is marked. \textbf{(B)}~Membership: all three steps are reachable,
the parallel legs are not necessary. \textbf{(C)}~Over $k$ parallel routes, a
$\seek$-only carry retains all $k$ while $\nec$ retains only the load-bearing
core---the over-retention $\seek$ alone incurs. \textbf{(D)}~The
$\seek-\nec$ gap over $(\text{routes},\text{depth})$.}
\label{fig:panel7}
\end{figure*}
