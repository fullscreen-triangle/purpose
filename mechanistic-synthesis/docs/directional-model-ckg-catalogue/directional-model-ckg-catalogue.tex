...
\title{\textbf{The Directional Pair:\\[3pt]
A Model Without Identity, a Catalogue Without Process,\\
and the Single Table They Jointly Evaluate}}
...
interpretive readings are confined to remarks on which no theorem depends.
\end{abstract}
...
An item is necessary for a target exactly when it \emph{dominates} some
reachable item---lies on every path from the target to it. Domination is
...
Every object in this paper is a finite weighted graph and every quantity is a
minimum cut, a reachability, a domination, or a convex optimum. Each theorem
is therefore checkable by direct computation on explicitly constructed and
randomly drawn instances, with minimum cuts computed exactly by maximum flow
\citep{fordfulkerson1956,edmondskarp1972}. We describe the checks the
development admits; the constructions are elementary and deterministic under a
fixed seed.

\begin{enumerate}[leftmargin=1.6em,itemsep=3pt]
\item \textbf{Floor} (\Cref{thm:floor}). Generate random contact graphs with a
positive minimum edge weight; verify every item's minimum cut against the
medium meets the floor and no cut is zero.

\item \textbf{Invariance} (\Cref{thm:invariant}). Apply random weighted
relabellings; verify each item's separation cost is preserved exactly while
its label permutes.

\item \textbf{Region-valuedness} (\Cref{thm:region}). Build the two-cluster
construction of the proof; verify the minimising side is not a singleton.

\item \textbf{Monotone non-return} (\Cref{thm:monotone}). Run walks that
revisit configurations; verify the record strictly increases and that revisited
configurations are distinguished by it.

\item \textbf{Self-blunting} (\Cref{thm:blunt}). Commit a sequence of cuts;
verify the instrument's uncommitted capacity strictly decreases and its record
strictly increases at each step, and that the two curves cross as capacity is
spent.

\item \textbf{Directional identity} (\Cref{thm:catalogue-table}). For random
graphs and every item, verify
$\cell(v)=\Tab(v,v,\Proc_{\mathrm{rest}}(v))$ as edge sets, and verify that
outputs of nontrivial processes are cuts of weight $\ge\flo$.

\item \textbf{Identity criterion} (\Cref{thm:termination-crit}). Construct the
three cases of \Cref{tab:theseus}; verify $\Char$ agrees in all three while
the record chain is continuous only in the first, and that the disassembly
stage leaves the unit open.

\item \textbf{Floor readout} (\Cref{thm:floor-readout}). Build chains of term
maps ordered by refinement; verify the realised system floor is monotone
non-decreasing along each chain.

\item \textbf{Multiplicative composition} (\Cref{thm:multiplicative}).
Compose random probe chains; verify measured composite power equals
$1-\prod_i(1-\pot_i)$ to machine precision.

\item \textbf{Saturation dichotomy} (\Cref{cor:saturation}). Test canonical
power sequences (constant, harmonic, geometric, inverse-square); verify the
divergent-sum sequences drive the residual toward zero and the convergent-sum
sequences plateau strictly above it.

\item \textbf{Coherence triangle} (\Cref{thm:triangle}). Generate random
support graphs; verify every acyclic graph and every $2$-cycle fails the
single-removal robustness check and every strongly connected triangle at
$\theta>\tfrac12$ passes.

\item \textbf{Water-filling} (\Cref{thm:waterfill}). Compare a bisection
solver on the scalar price against a convex-program reference on random
instances; verify agreement, verify the price is nonincreasing in the budget
and nondecreasing in the probe count, and verify the sharp dropout boundary at
$\gain_i'(0)=\price$.

\item \textbf{Separation of operators}
(\Cref{thm:separation-operators}). Build the redundant-reachable witness;
verify an item that $\seek$ marks reachable is marked purposeless by $\nec$,
and measure the over-retention gap as the number of parallel routes grows.

\item \textbf{Closure versus threshold} (\Cref{thm:closure-stronger}).
Build two-cluster process graphs; verify the confidence criterion is met after
one probe while an uninvoked probe still reaches a distinct class.

\item \textbf{Convergent/contested dichotomy} (\Cref{thm:decline}). Run
determinations to closure on random registries; verify every run terminates in
exactly one of the two states.

\item \textbf{Query separation} (\Cref{thm:query-separation},
\Cref{thm:corpus-separation}). Build the $\Gph_1,\Gph_2$ pair of the proof;
verify the contact relations are identical as assertion sets while the
admissibility verdicts differ at $\eps=0$, and verify that any map computed
from the assertion set alone returns the same value on both.
\end{enumerate}

Each check is a finite computation whose predicted and measured outcomes agree
exactly for identities and within the stated bound for inequalities. The suite
is a guard against error and an exhibition of the quantitative shape of each
result, not a substitute for the proofs, which hold for every finite contact
...
\item \textbf{Necessity is domination, not reachability.}
\Cref{prop:nec-subset} gives only the inclusion, and
\Cref{thm:separation-operators}(iii) exhibits its strictness. The exact
criterion is \Cref{prop:domination}. A system computing reachability and
calling it necessity over-retains.

\item \textbf{The floor readout is monotone, not calibrated.}
\Cref{rem:quality-honest} states this: a rising floor indicates refinement,
but a high floor is not by itself evidence of a good term map, and optimising
it directly would be an error.
